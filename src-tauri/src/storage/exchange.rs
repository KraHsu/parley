...
mod tests {
    ...
    fn downgrade_to_v5(storage: &Storage) {
        ...
        db.batch_execute(
            "DROP TABLE backend_credential_scopes; DROP TABLE backend_credential_salts; DROP TABLE conversation_context; ALTER TABLE vocabulary_occurrences DROP COLUMN backend; PRAGMA user_version=5;",
        )
        .unwrap();
    }
    ...
    #[test]
    #[ignore = "requires PARLEY_TEST_LEARNING_EXPORT generated by the Web serializer"]
    fn runtime_web_export_imports_and_roundtrips_without_truncation() {
        let path = std::env::var("PARLEY_TEST_LEARNING_EXPORT").unwrap();
        let input = Backup::parse(&std::fs::read(path).unwrap()).unwrap();
        let storage = Storage::memory();
        let (_, pending) = storage.vocabulary_preview(input.clone()).unwrap();
        storage.vocabulary_import(&pending, &id(), false).unwrap();
        let output = storage.vocabulary_backup(true).unwrap();
        output.validate().unwrap();
        assert_eq!(
            serde_json::to_value(input.entries).unwrap(),
            serde_json::to_value(output.entries).unwrap()
        );
    }
    #[test]
    fn shared_web_fixture_preserves_sources_both_schedules_history_and_trash() {
        let backup =
            Backup::parse(include_bytes!("../../../fixtures/learning-exchange.json")).unwrap();
        let storage = Storage::memory();
        let (_, pending) = storage.vocabulary_preview(backup.clone()).unwrap();
        assert_eq!(
            storage
                .vocabulary_import(&pending, &id(), false)
                .unwrap()
        ...
        let exported = storage.vocabulary_backup(true).unwrap();
        assert_eq!(
            serde_json::to_value(exported.entries).unwrap(),
            serde_json::to_value(&backup.entries).unwrap()
        );
        let (preview, _) = storage.vocabulary_preview(backup).unwrap();
        assert_eq!(preview.duplicates, 2);
    }
    #[test]
    fn schedule_migration_preserves_old_cards_and_review_foreign_keys() {
        let before = populated().vocabulary_backup(true).unwrap();
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("workspace.sqlite3");
        {
            let mut db = super::super::typed::connect(path.to_str().unwrap()).unwrap();
            for migration in &super::super::MIGRATIONS[..8] {
                db.batch_execute(migration).unwrap();
            }
            for entry in &before.entries {
                insert_record(&mut db, entry, false).unwrap();
            }
        }
        let storage = Storage::open(&path).unwrap();
        let after = storage.vocabulary_backup(true).unwrap();
        assert_eq!(
            serde_json::to_value(before.entries).unwrap(),
