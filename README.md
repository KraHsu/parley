...
已发布 **0.4.0-alpha.2** 预览版；当前源码为 **0.4.0-alpha.3**，新增伴随会话管理和 Web 表达复习，升级记录见[安装验证](docs/UPGRADE_VALIDATION.md)。Web 已上线；桌面端验证过 Ubuntu 24.04 amd64，[Windows x64 安装包](docs/WINDOWS.md)已通过安装、窗口启动、覆盖安装和卸载检查，macOS 目前只通过构建检查。API 适配已实现，真实厂商测试还没做，范围见[兼容记录](docs/BACKEND_COMPATIBILITY.md)。
