...
状态：**开发中，Diesel 业务迁移、Linux 开发包 API 学习闭环与双 CLI 原生终端同步已验证，各后端实测与发布验收继续推进，尚未达到首版交付条件**。基于 v0.3.0 / `55cb6a5`；文档与官方接口核对日期：2026-09-14。作为 v0.4.0 的主线，保留 Vue + Rust + Tauri 技术栈。具体增量与验证见 [实现记录](MULTI_BACKEND_IMPLEMENTATION.md)。

2026-09-14 技术决策更新：用户选择 **Diesel** 作为 Rust 与 SQLite 的业务访问层。P1 增加存储层迁移，新增 API 工作同时遵守此决定；完整交付范围保持不变。

2026-09-14 范围调整：按用户最新要求，**暂缓真实 API 厂商实测，优先交付仅 API 的 Web 端与 GitHub Pages**。此阶段不以缺少 API 凭据阻塞 Web 开发或发布；真实服务结果仍标为未验证。Web 使用独立的浏览器存储与静态构建，保留双面板对话、解释、词句收藏、复习和备份；本机 CLI 功能保留在桌面端。实现及部署范围见 [Web 使用说明](WEB.md)。
...
GUI 保留官方 `codex app-server` 接入及其管理的认证；沿用用户选择的本机 Codex 和已有登录。已实现按配置独立管理进程、账号读取、模型列表与取消，事件带配置 ID 和版本。不同配置使用独立工作目录；使用相同本机登录目录时共享官方账号，不自动创建独立账号或复制凭据。终端继续直接运行原生 Codex TUI，保留快捷键、工具、历史和官方登录交互。App Server 本身提供产品集成所需的认证、线程、模型与流式能力。[官方 App Server 文档](https://learn.chatgpt.com/docs/app-server)
...
上表记录实施前的耦合。当前已拆出独立 workspace store、通用 chat store、后端配置 store 与按配置管理的 Codex 连接 registry。workspace 负责本地文档、历史、偏好与写入顺序，chat 负责活动请求、事件路由和切换保护；Codex 已复用通用 turn 记录、发布和前端事件处理，发送准备与事件保存已移至阻塞任务。Codex 通知使用有界队列并约每 50 毫秒合并写入，RPC 回执不等待通知保存；真实首段输出后取消与另一面板完成已验证。`BackendState` 统一持有 Codex registry 与 API/Claude 请求状态，前端共用发送、停止与关闭入口；请求携带的配置类型和版本在适配器内核对，不能用于绕过保存的配置。协议认证和终端来源继续保留各自接口。不能只把 `useCodexStore` 改名，内部仍依赖一个全局账号或线程。
...
当前数据库版本为 7，已完成从 v3/v4/v5/v6 的升级测试。迁移 004、005、006、007 已落地；历史迁移不改写：
...
| `007_conversation_context.sql`（已实施） | 显式带入的可见文字快照                           | 不复制上游会话 ID、凭据或协议私有内容，创建与快照在同一事务内完成   |
...
- [x] Claude 原生终端新轮次自动同步；历史读取范围与不支持条件在 UI 和文档中明确。
- [x] 纯 API 模式无需安装 CLI；密钥有安全存储与仅本次会话选项。
- [ ] 重启、换后端、换账号、换配置后不串会话、不重复发送、不丢草稿。
- [ ] 旧词句、来源、标签和复习记录迁移成功；v1/v2 备份往返与幂等通过。
- [ ] 流式正文、错误与终态一致，关闭时间有界，无遗留的 Parley 托管进程。
- [x] 仓库要求的格式、类型、Rust 检查和现有测试通过；新增协议与迁移测试通过。
