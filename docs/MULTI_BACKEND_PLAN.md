...
按单人集中开发估算，共 **22–32 个工作日，约 5–7 周**；不包含等待各服务账号、API 凭据或平台设备。先跑通旧 Codex 与一个真实 API 的完整纵向流程，再扩展接口，不一次性重写整个应用。

| 阶段                     | 工作量 | 交付                                                                                              | 验收门槛                                                 |
| ------------------------ | ------ | ------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| P0：协议与能力验证       | 2–3 天 | 固定 CLI 版本、API 事件契约、Claude 认证与工具限制、终端 hooks 小实验；记录 Gemini 无状态多轮方案 | 流式、续聊、取消与有效限制有证据；未验证能力明确记录     |
| P1：通用会话层与迁移     | 3–4 天 | 新契约、store 拆分、配置及会话迁移、Codex 适配                                                    | 旧数据可开、旧 Codex 双会话可续聊，取消和关闭不退化      |
