...
安装包最终核对：精简 Ubuntu 镜像的 dpkg 默认排除 `/usr/share/doc/*`，在测试容器中增加仅针对 Parley 的文档包含规则后重装，安装指南、后端指南和兼容记录的 SHA256 均与源码一致。规范化步骤将临时目录的 0700 改为归档根目录 0755；最终包再次安装，系统根目录仍为 0755，包内可执行文件与前述完整 UI 验证使用的文件一致。容器已停止，测试数据副本仅保存在临时目录。

## 2026-09-14：三平台 CI 与安装版拼音输入

当前开发实现已推送至 `validation/multi-backend-alpha`。首次 CI（`a24ab8c`，run 34859678767）中 Linux 与 macOS 的格式、类型、Clippy、测试及 Tauri 调试构建通过；Windows 的 Rust 测试发现两个夹具写死 Unix CLI 路径，触发生产代码的绝对路径校验。夹具改用平台临时目录下的路径，保留路径同步、空白归一化、修订号幂等和重启恢复断言；没有放宽生产校验或跳过 Windows 测试。

继续使用上一节点的实际 deb 安装及隔离数据，以 IBus 1.5.29、ibus-libpinyin 1.15.7 和 GTK 3 输入模块完成原生输入验证。Xvfb 测试环境需要安装 `ibus-gtk3` 并选择 libpinyin；这些配置仅属于测试环境，没有强制用户改用 IBus。

- 在主聊和助手输入框逐键输入拼音，候选框显示中文；组合输入时按回车只结束组合，数据库 turn 数保持 6，回环服务没有收到任何生成请求。
- 使用空格选择候选，主聊输入“中文草稿”，助手输入“学习语言”、Shift+Enter 换行及“草稿”；正常关闭并重启后分别恢复为 `中文草稿` 和 `学习语言\n草稿`，只读数据库与界面一致。
- 本次中文由实际 IBus 拼音引擎组成，不使用剪贴板注入；候选框截图来自原生 X11 显示器。该证据限于 Linux X11/IBus，不外推其他输入法、Wayland 或其他平台。
