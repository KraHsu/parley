...
当前未提供应用内导入/导出按钮。仅备份 Parley 数据库可以恢复离线记录；继续原模型上下文还需要原账号及 Codex 自身保存的对应 thread。不要删除 Codex 自身记录后期待 Parley 自动重建它。
...
- Windows/macOS 尚未真机验证。词句收藏、注释与复习功能仍待实现，不属于本轮已有工作区数据的保存范围。
