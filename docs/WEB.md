...
- Web 新备份使用独立的 `parley-web` 逐行格式（format 3，兼容 format 2），并保留旧 v1 JSON 导入，包含可见聊天和学习数据；不包含密钥、隐藏推理与协议私有续聊对象。导入前显示数量并确认替换当前数据。完整工作区备份仍用于 Web 自身；跨端迁移使用词句页的“词句迁移 · Web ↔ 桌面”入口。该入口导入桌面词句 JSON v1/v2/v3，导出共用 v3，保留多来源、回收站、双向卡片和历史。导入先预览再合并，不替换对话、设置或当前密钥。
- Web 原生识义与表达卡片按 1/3/7/14/30/60 天递进，再学一次安排到 10 分钟后。导入的桌面卡片保留原排程版本；旧版最后一级仍为 30 天。Web 可以继续识义与表达复习，支持暂停、恢复和撤销最近评分；排程和历史可再次导出。词句原句快照随备份保存。页面打开后可在无 API 凭据时操作本地词句；未实现离线缓存整个网站。
...
词句迁移的格式、冲突规则和升级边界见[续聊、重连与词句迁移](CONTINUITY_AND_TRANSFER.md)。

`0.4.0-alpha.3` 补齐表达复习。先显示释义，回忆目标表达后揭晓答案；写入失败保留原题与排程。浏览器回归覆盖失败重试、刷新恢复、撤销和导出评分历史，详见[续聊、重连与词句迁移](CONTINUITY_AND_TRANSFER.md)。
