...
- Web 原生识义卡片按 1/3/7/14/30/60 天递进，再学一次安排到 10 分钟后。导入的桌面卡片保留原排程版本；旧版最后一级仍为 30 天。Web 可以继续识义复习，表达卡片及其历史保留并可再次导出。词句原句快照随备份保存。页面打开后可在无 API 凭据时操作本地词句；未实现离线缓存整个网站。
