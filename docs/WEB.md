...
- Web 备份使用独立的 `parley-web` v1 格式，包含可见聊天和学习数据；不包含密钥、隐藏推理与协议私有续聊对象。导入前显示数量并确认替换当前数据。桌面 JSON v1/v2 备份暂不直接互通，不会静默误解析。
...
`.github/workflows/pages.yml` 对 PR 检查格式、前端测试与 Web 构建；仅 push 或手动运行可以部署。仓库 Pages 使用 GitHub Actions 发布源，部署环境为 `github-pages`。开发阶段允许 `validation/multi-backend-alpha` 与 `main` 发布；合并后可移除验证分支触发条件。工作流不需要任何 API Key。[GitHub Pages 官方工作流说明](https://docs.github.com/en/pages/getting-started-with-github-pages/using-custom-workflows-with-github-pages)
